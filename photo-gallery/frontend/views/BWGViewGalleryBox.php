      ...
      <div class="bwg_btn_container">
        <div class="bwg_ctrl_btn_container">
          ...
          if ( $params['show_image_counts'] ) {
            ?>
            <span
              class="bwg_image_count_container bwg_ctrl_btn" <?php echo !$params['popup_enable_ctrl_btn'] ? "style='position:relative'" : "" ?>>
          ...
          if ( $params['popup_enable_ctrl_btn'] ) {
            $share_url = add_query_arg(array(
                                         'curr_url' => urlencode($current_url),
                                         'image_id' => $current_image_id,
                                       ), WDWLibrary::get_share_page()) . '#bwg' . $gallery_id . '/' . $current_image_id;
            ?>
            ...
            <?php if ( $params['popup_enable_fullscreen'] ) {
              if ( !$params['popup_fullscreen'] ) {
                ?>
                <i title="<?php echo __('Maximize', BWG()->prefix); ?>"
                   class="bwg-icon-expand bwg_ctrl_btn bwg_resize-full"></i>
                <?php
              }
              ?>
              <i title="<?php echo __('Fullscreen', BWG()->prefix); ?>"
                 class="bwg-icon-arrows-out bwg_ctrl_btn bwg_fullscreen"></i>
            <?php }
            if ( $params['popup_enable_info'] ) { ?>
              <i title="<?php echo __('Show info', BWG()->prefix); ?>"
                 class="bwg-icon-info-circle bwg_ctrl_btn bwg_info"></i>
            <?php }
            if ( $params['popup_enable_comment'] ) { ?>
              <i title="<?php echo __('Show comments', BWG()->prefix); ?>"
                 class="bwg-icon-comment-square bwg_ctrl_btn bwg_comment"></i>
            <?php }
            if ( $params['popup_enable_rate'] ) { ?>
              <i title="<?php echo __('Show rating', BWG()->prefix); ?>"
                 class="bwg-icon-<?php echo $theme_row->lightbox_rate_icon; ?> bwg_ctrl_btn bwg_rate"></i>
            <?php }
            if ( $params['popup_enable_zoom'] ) { ?>
              <i title="<?php echo __('Zoom in-out', BWG()->prefix); ?>"
                 class="bwg-icon-search bwg_ctrl_btn bwg_zoom"></i>
            <?php }
            $is_embed = preg_match('/EMBED/', $current_filetype) == 1 ? TRUE : FALSE;
            $share_image_url = str_replace(array(
                                             '%252F',
                                             '%25252F',
                                           ), '%2F', urlencode($is_embed ? $current_thumb_url : BWG()->upload_url . rawurlencode($current_image_url)));
            if ( $params['popup_enable_facebook'] ) {
              ?>
              <a id="bwg_facebook_a"
                 href="https://www.facebook.com/sharer/sharer.php?u=<?php echo urlencode($share_url); ?>"
                 target="_blank" title="<?php echo __('Share on Facebook', BWG()->prefix); ?>">
                <i title="<?php echo __('Share on Facebook', BWG()->prefix); ?>"
                   class="bwg-icon-facebook-square bwg_ctrl_btn bwg_facebook"></i>
              </a>
              <?php
            }
            if ( $params['popup_enable_twitter'] ) {
              ?>
              <a id="bwg_twitter_a" href="https://twitter.com/share?url=<?php echo urlencode($share_url); ?>"
                 target="_blank" title="<?php echo __('Share on Twitter', BWG()->prefix); ?>">
                <i title="<?php echo __('Share on Twitter', BWG()->prefix); ?>"
                   class="bwg-icon-twitter-square bwg_ctrl_btn bwg_twitter"></i>
              </a>
              <?php
            }
            if ( $params['popup_enable_pinterest'] ) {
              ?>
              <a id="bwg_pinterest_a"
                 href="http://pinterest.com/pin/create/button/?s=100&url=<?php echo urlencode($share_url); ?>&media=<?php echo $share_image_url; ?>&description=<?php echo $current_image_alt . '%0A' . $current_image_description; ?>"
                 target="_blank" title="<?php echo __('Share on Pinterest', BWG()->prefix); ?>">
                <i title="<?php echo __('Share on Pinterest', BWG()->prefix); ?>"
                   class="bwg-icon-pinterest-square bwg_ctrl_btn bwg_pinterest"></i>
              </a>
              <?php
            }
            if ( $params['popup_enable_tumblr'] ) {
              ?>
              <a id="bwg_tumblr_a"
                 href="https://www.tumblr.com/share/photo?source=<?php echo $share_image_url; ?>&caption=<?php echo urlencode($current_image_alt); ?>&clickthru=<?php echo urlencode($share_url); ?>"
                 target="_blank" title="<?php echo __('Share on Tumblr', BWG()->prefix); ?>">
                <i title="<?php echo __('Share on Tumblr', BWG()->prefix); ?>"
                   class="bwg-icon-tumblr-square bwg_ctrl_btn bwg_tumblr"></i>
              </a>
              <?php
            }
            if ( $params['popup_enable_fullsize_image'] ) {
              ...
              <a id="bwg_fullsize_image"
                 href="<?php echo !$is_embed ? BWG()->upload_url . urldecode($current_image_url) : urldecode($current_image_url); ?>"
                 data-elementor-open-lightbox="no" target="_blank">
                <i title="<?php echo __('Open image in original size.', BWG()->prefix); ?>"
                   class="bwg-icon-sign-out bwg_ctrl_btn"></i>
              </a>
              <?php
            }
            if ( $params['popup_enable_download'] ) {
              ...
              ?>
              <a id="bwg_download" <?php if ( $is_embed ) { ?> class="bwg-hidden" <?php } ?>
                 href="<?php echo $download_href; ?>" target="_blank" download="<?php echo end($current_image_arr); ?>">
                <i title="<?php echo __('Download original image', BWG()->prefix); ?>"
                   class="bwg-icon-download bwg_ctrl_btn"></i>
              </a>
              <?php
            }
            if ( function_exists('BWGEC') && $params['popup_enable_ecommerce'] == 1 ) {
              ?>
              <i title="<?php echo __('Ecommerce', BWG()->prefix); ?>"
                 style="<?php echo $pricelist_id == 0 ? "display:none;" : ""; ?>"
                 class="bwg-icon-shopping-cart bwg_ctrl_btn bwg_ecommerce"></i>
              <?php
            }
          }
          ?>
        ...
        <?php
        if ( $params['popup_enable_ctrl_btn'] || $params['show_image_counts'] ) {
          ?>
