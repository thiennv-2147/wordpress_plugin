...
jQuery(document).on('change','div[type="type_date_fields"]', function() {
  jQuery(this).next(".fm-not-filled").remove();
  ...
  jQuery("#form" +form_id+ " div[type='type_date_fields']").each(function() {
    ...
    if ( !get_data_min_day ) {
      ...
      if ( !get_data_day && !get_data_month && !get_data_year ) {
        ...
        if ( isNaN(get_data_full_convert) || get_data_full_convert - get_min_data_full_convert < 0 ) {
          success = false;
          jQuery("#check_min_date_" + wdid + "_" + form_id).remove();
          parent_div.append("<div id='check_min_date_" + wdid + "_" + form_id + "' class='fm-not-filled'>" + jQuery(this).attr("data-min-date-alert") + "</div>");
